# Unitree Go2 인도 장애물 회피 시스템 — 마스터 계획 (v3.1.1)
...
> **v3.1.1 (2026-08-01) 소정정 + 대장 초판 등재.**
> ⑩ §4.1에 `UNVERIFIED` 상태 정의 추가 (표에서 쓰이나 미정의였음)
> ⑪ §6.3 재작성 — 실코드 파라미터명 정정(`stop_ttc` 1.0 / `fast_stop_ttc` 2.0 /
>   `cpa_growth` / `roi_x_max`) + **중첩 스키마 폐기**: 런타임 yaml은 평탄 유지,
>   메타데이터는 `params_ledger.yaml` 단일 정본 (rclpy 중첩 dict 미지원 + §4 정본 선언과 중복)
> ⑫ §4.4 양보 발동 지상속도화·발동 거리 상한 6 m(커밋 92591f6) 반영,
>   워치독 연장 상한(`YIELD_WAIT_NEAR_MAX` 20 s) 등재
> ⑬ §6.6 실파일 위치 정정(`unitree_go2_description` 소속 xacro) + L1/L2 xacro 인자화 권고
> ⑭ `config/params_ledger.yaml` **초판 등재 완료** + `test_params_ledger.py` 드리프트 차단
>   (§9.1 항목 3 선행 이행)
...
| `UNVERIFIED` | 값 이전에 **전제(검출 가능성·물리 특성)가 미확인** | 해당 측정 전까지 계획 근거로 인용 금지 (v3.1.1 정의 추가) |
...
| 양보 진입 접근속도 창 | 래치 (−2.0, −0.5) 상대 vx / **발동은 액터 지상속도 ≤ −0.3 + 거리 ≤ 6 m** | **PENDING** | v3.1.1: 발동 기준 지상속도화 + 거리 상한 (92591f6 — 조기 발동·에피소드 분절 해소). 클러터 노이즈가 L2 밀도에서 변화. 측정 6C 병행 |
...
| 양보 워치독 연장 상한 | 20 s (래치 생존 + 접근 지속 시) | CONFIRMED | 센서 무관. 코드 `YIELD_WAIT_NEAR_MAX` (v3.1.1 등재) |
...
### 6.3 🔴 파라미터 메타데이터 — `params_ledger.yaml` 단일 정본 (v3.1.1 재작성)

v3 초안은 `safety_stop.yaml` 자체를 중첩 스키마(value/status/…)로 바꾸는 안이었다.
**폐기한다**: rclpy는 중첩 dict 파라미터를 지원하지 않아 로딩이 깨지고, §4가 이미
`params_ledger.yaml`을 정본으로 선언했으므로 yaml 중첩은 이중 기록이다.

```
v3 초안의 파라미터명 정정 (실코드 기준):
  safety_stop.yaml   stop_ttc: 1.0 (보행자 정지) · fast_stop_ttc: 2.0 (fast-class)
                     cpa_growth: 0.4        (초안 표기 cpa_expansion_rate)
  self_filter.yaml   roi_x_max: 15.0        (초안 표기 roi_max)
  ※ §1.4 재유도식의 stop_ttc 심볼은 fast_stop_ttc 를 가리킨다.

확정 구조 (v3.1.1 등재 완료):
  · 런타임 yaml(safety_stop / self_filter)은 평탄 스칼라 유지
    — 노드 로딩·기존 test_config_parity 무변경
  · 메타데이터(status / basis_tier / formula / inputs / sensor_generation /
    depends_on / source)는 config/params_ledger.yaml 레코드로 기록
  · test_params_ledger.py: 대장 value ↔ 코드 값(yaml 파라미터·모듈 상수·
    GapRules 기본값) 드리프트 차단 + 스키마 검사 (④는 sensor_generation 필수)
  · cpa_growth 는 C1 도입 시 삭제 → cp_quantile 테이블 참조로 대체 (유지)
  · 원칙 9 런타임 가드: 게이트가 기동 시 대장을 읽어 자기 안전 파라미터의
    status 확인 — 실기체 모드에서 PENDING/INVALID 발견 시 기동 거부 (P8')
```
...
**실파일 위치 정정 (v3.1.1)**: 라이다 센서 블록은 `go2_simulation`이 아니라
`ros2_ws/src/unitree_go2_ros2/unitree_go2_description/urdf/lidar_4D_lidar.xacro`에
있다 (외부 유래 vendored 패키지 — 리포에 포함돼 직접 수정 가능). 권고:
`update_rate / min_range / samples`를 xacro 인자(기본값 L2)로 빼고 launch에
`lidar_gen:=l1|l2`를 노출한다 — Phase 3.5의 핵심 산출물인 L1↔L2 델타 문서
(`l1_to_l2_delta.md`)를 스위치 한 번으로 재현하기 위함이며, `real_l1` 프로파일을
DEPRECATED로 보존하는 §6.5와 같은 취지다.
...
3. ~~`params_ledger.yaml` 도입~~ **v3.1.1 선행 완료** — 재기준화 결과로 status만 갱신
   (yaml 스키마 변경 없음, §6.3)
...
ros2_ws/src/perception_avoidance/
  ...
  config/
    params_ledger.yaml            # ★C7 — 정본, 기계 판독 (v3.1.1 초판 등재 완료)
    cp_quantiles.yaml             # ★신규 C1
    safety_stop.yaml              # [v3.1.1: 평탄 유지 — 메타데이터는 params_ledger]
